    <template>
        <div class="flex flex-col h-full p-4">
            ...
            <NuxtLink to="/" class="mb-6 flex items-center justify-center xl:justify-start gap-2.5">
                <div
                    class="w-9 h-9 shrink-0 bg-gradient-to-br from-[#f02c56] to-purple-600 rounded-full flex items-center justify-center">
                    <Icon name="mdi:hanger" class="w-5 h-5 text-white" />
                </div>
                <span
                    class="hidden xl:inline text-xl font-bold tracking-tight text-gray-900 dark:text-neutral-100">Styli</span>
            </NuxtLink>
            ...
            <nav class="flex flex-col space-y-2">
                    ...
                    <button v-if="profileStore.isLoggedIn" @click="$emit('open-notifications')" class="nav-button relative">
                        <div class="relative">
                            ...
                            <span v-if="unreadCount > 0"
                                class="absolute -top-1 -right-1 block h-2.5 w-2.5 rounded-full bg-brand"></span>
                        </div>
                ...
                <ClientOnly>
                    <NuxtLink v-if="profileStore.isLoggedIn" to="/messages" class="nav-button relative" active-class="active">
                        <div class="relative">
                ...
                <button @click="$emit('open-cart')" class="nav-button relative">
                    <div class="relative">
                        ...
                        <span v-if="cartCount > 0"
                            class="absolute -top-1 -right-1 min-w-[16px] h-4 rounded-full bg-brand text-white text-[9px] font-bold flex items-center justify-center px-0.5">{{
            ...
            <div class="mt-auto relative" ref="menuRef">
                <ClientOnly>
                    ...
                    <button v-if="profileStore.isLoggedIn" @click="menuOpen = !menuOpen" class="nav-button w-full relative"
                        :class="menuOpen ? 'bg-gray-100 dark:bg-neutral-800' : ''">
                        <div class="relative">
                            <!-- Avatar -->
                            <Avatar :username="profileStore.me?.username || 'Shopper'" :avatar="profileStore.me?.avatar ?? undefined" size="sm" />
                ...
                <Transition name="menu-pop">
                    <div v-if="menuOpen"
                        ...
                        <!-- Profile -->
                        <NuxtLink :to="'/profile/' + profileStore.me?.username" @click="menuOpen = false" class="menu-item">
                        ...
                        <NuxtLink v-if="sellerStore.hasSellers" to="/seller/dashboard" @click="menuOpen = false"
                            class="menu-item">
