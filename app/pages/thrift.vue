        ...
        <div class="w-full pb-20 md:pb-0">
            ...
            <div class="mb-5">
                <div class="flex items-center gap-2 mb-1">
                    <div class="w-8 h-8 rounded-full bg-emerald-100 dark:bg-emerald-950 flex items-center justify-center">
                        <Icon name="mdi:tshirt-crew-outline" size="18" class="text-emerald-600 dark:text-emerald-400" />
                    </div>
                    <h1 class="text-[22px] font-bold text-gray-900 dark:text-neutral-100">Thrift Store</h1>
                </div>
            ...
            <div class="relative mb-5">
                ...
                <input
                    ...
                    class="w-full pl-9 pr-4 py-2.5 bg-gray-100 dark:bg-neutral-800 rounded-xl text-[14px] text-gray-900 dark:text-neutral-100 placeholder-gray-400 dark:placeholder-neutral-500 focus:outline-none focus:ring-2 focus:ring-emerald-400/40 transition-shadow"
                />
            ...
            <div v-else-if="error && !products.length" class="flex flex-col items-center justify-center py-24 gap-4">
                ...
                <button @click="load(true)" class="px-5 py-2 bg-emerald-500 text-white text-sm font-semibold rounded-full hover:bg-emerald-600 transition-colors">
                    {{ $t('common.tryAgain') }}
                </button>
            </div>
            ...
            <div v-else-if="!isLoading && !products.length && searchInput" class="flex flex-col items-center justify-center py-24 gap-3">
                ...
                <button @click="searchInput = ''" class="text-sm font-semibold text-emerald-500 hover:text-emerald-600 transition-colors">Clear search</button>
            </div>

            <!-- Empty thrift store -->
            <div v-else-if="!isLoading && !products.length" class="flex flex-col items-center justify-center py-24 gap-4">
                <Icon name="mdi:hanger" size="56" class="text-gray-300 dark:text-neutral-600" />
                <p class="text-[16px] font-semibold text-gray-600 dark:text-neutral-400">No thrift items yet</p>
                <p class="text-[13px] text-gray-400 dark:text-neutral-500">Check back soon for pre-loved fashion</p>
            </div>
