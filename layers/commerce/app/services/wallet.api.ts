...
export class WalletApiClient extends BaseApiClient {
  ...
  async getTransactions(params?: { limit?: number; offset?: number }) {
    ...
    return this.request(`/api/commerce/wallet/transactions${query}`, { method: 'GET' })
  }
  async addFunds(amount: number) {
    return this.request('/api/commerce/wallet/add-funds', { method: 'POST', body: { amount } })
  }
  async withdraw(amount: number, bankAccount: any) {
    return this.request('/api/commerce/wallet/withdraw', { method: 'POST', body: { amount, bankAccount } })
  }
}
