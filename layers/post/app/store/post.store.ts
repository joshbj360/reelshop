...
export const usePostStore = defineStore('post', () => {
  ...
  const addLikedPostsByUser = (newPosts: any[], username: string) => {
    ...
    const newIds = newPosts.map(p => p.id)
    userLikedPosts.value.set(username, [...new Set([...existingIds, ...newIds])])
  }

  const deletePost = (id: string) => {
    posts.value.delete(id)
    userPosts.value.forEach((ids, key) => {
      userPosts.value.set(key, ids.filter(postId => postId !== id))
    })
  ...
  return {
    posts, userPosts, userLikedPosts, isLoading, error,
    getPostById, getPostsByUsername, getLikedPostsByUsername,
    addPosts, addLikedPostsByUser, deletePost, updatePost,
