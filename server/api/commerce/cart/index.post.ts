...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (!variantId) throw new UserError('INVALID_INPUT', 'variantId is required', 400)
    const result = await cartService.addToCart(user.id, Number(variantId), Number(quantity))
