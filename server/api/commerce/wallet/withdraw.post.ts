...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (!amount || isNaN(Number(amount))) throw new UserError('INVALID_INPUT', 'amount is required', 400)
    if (!bankAccount) throw new UserError('INVALID_INPUT', 'bankAccount details are required', 400)
    const ipAddress = getHeader(event, 'x-forwarded-for') || getClientIP(event) || 'unknown'
    const userAgent = getHeader(event, 'user-agent') || 'unknown'
    const result = await walletService.withdraw(sellerProfile.id, Number(amount), bankAccount, ipAddress, userAgent)
